/*
 * File: NimMiniMaxAdvanced_Client.java
 * --------------
 * This program simulates a simple variant of the game of Nim using 
 * minimax algorithm.
 * In this version, the game starts with a pile of 17 coins on a table.
 * Players then take turns removing 1, 2,3, or 4 coins from the pile.
 * The player who has odd coins in his pile loses.
 */

import java.util.ArrayList;
import java.util.Scanner;

public class NimMiniMaxAdvanced_Client extends TwoPlayerGame {

  /* Main program */

  public static void main(String[] args) {
     new NimMiniMaxAdvanced_Client().run();
  }
  ...
  private int nCoins, pPile, cPile;    /* Number of coins left on the table */
  ...
  public void run() {
    ...
    while (nCoins > 1) {
      ...
      if (currentPlayer == Player.HUMAN) {
        int nTaken = getUserMove().getMove();
	nCoins -= nTaken;
	pPile += nTaken;
      } else {
  ...
  public void announceResult() {
    if (nCoins == 0 && cPile%2 ==0) {
       System.out.printf("\nYou took the last coin.  You lose.");
       System.out.printf("\nYour Pile = %d", pPile);
       System.out.printf("\nMy Pile = %d", cPile);
    } else {
       System.out.printf("\nThere is only one coin left.");
       if (currentPlayer == Player.HUMAN && (pPile+1)%2 == 0) {
         System.out.printf("\nI win.\n");
         System.out.printf("\nYour Pile = %d", pPile+1);
         System.out.printf("\nMy Pile = %d\n", cPile);
       } else {
